...
module View =
    ...
    let private chat (actions: ViewActions) (dispatch: ClientMsg -> unit) (model: ClientModel) : TemplateResult =
        ...
        // forwarded credential is its own badge naming whose it is, not a clause; and each line
        ...
        let sandboxStartFacts (s: WorkSandboxStarted) =
            ...
            let forwarded =
                ...
                | names ->
                    let whose =
                        match s.CredentialOwner with
                        | Some owner -> sprintf " (%s)" (CredentialFor.token owner)
                        | None -> ""
                    names
                    |> List.map (fun name ->
                        html $"""<span class="{Style.actNoteCred}" data-act-fact="forwarded">{name}{whose}</span>""")
            let realisation =
                s.Realisation
                |> List.map (fun line ->
