    ...
    testList "Telemetry E2E (real Host, stub collector, no WebRTC)" [
        testCaseAsync "a completed turn's usage reaches the collector over OTLP" <|
            async {
                ...
                let! host = Host.startFull (fun () -> Some (usageAgent usage)) (fun () -> None) None None None None None None None sink None McpClient.McpConnections.none None sessionId None "" None false None 0
