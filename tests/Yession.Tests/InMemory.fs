    ...
    testList "In-memory transport (cheap E2E through the real Host)" [
        ...
        testCaseAsync "a peer's cursor presence relays to others in any field and clears on disconnect" <|
            async {
                ...
                do! a.Channel.Close ()
                do! b.Runner.WaitFor (fun m -> not (Map.containsKey (PeerRef a.Hello.PeerId) m.Presence))
                Expect.isFalse (Map.containsKey (PeerRef a.Hello.PeerId) (b.Runner.Model ()).Presence) "A's caret vanishes when she disconnects"
                do! host.Stop ()
            }
        ...
        testCaseAsync "a command targeted at a named sandbox runs in that sandbox, not the default one" <|
            async {
                ...
                let! started = host.Sandboxes.Ensure (Authority.agentFor (Principal.Peer (PeerId.create "ada" |> expect))) None test SandboxRequest.defaults
