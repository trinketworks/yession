...
/// What a start runs on: the agent, for a person.
let private starter : Authority = Authority.agentFor (Principal.User (UserId.create "ada" |> expect))
    ...
    testList "ensure semantics" [
        ...
        testCaseAsync "a sandbox coming up records starting before started, under one id" <|
            async {
                ...
                let! _ = sandboxes.Ensure starter (sandbox "test") (forwarding [])
        ...
        testCaseAsync "a sandbox that cannot come up records starting then failed, and no start" <|
            async {
                ...
                match! sandboxes.Ensure starter (sandbox "test") (forwarding []) with
        ...
        testCaseAsync "the same ask twice is the same sandbox, recorded once" <|
            async {
                ...
                let! first = sandboxes.Ensure starter (sandbox "test") (forwarding [])
                let! second = sandboxes.Ensure starter (sandbox "test") (forwarding [])
        ...
        testCaseAsync "an equivalent forwarding list is the same ask" <|
            async {
                ...
                let! _ = sandboxes.Ensure starter (sandbox "test") (forwarding [ "github" ])
                let! again = sandboxes.Ensure starter (sandbox "test") (forwarding [ " GitHub "; "github" ])
        ...
        testCaseAsync "a different forwarding is refused, naming both sides" <|
            async {
                ...
                let! _ = sandboxes.Ensure starter (sandbox "test") (forwarding [])
                match! sandboxes.Ensure starter (sandbox "test") (forwarding [ "github" ]) with
        ...
        testCaseAsync "a different spec is refused too, naming which part differs" <|
            async {
                let log = newLog ()
                let! sandboxes, built = registry log []
                let! _ = sandboxes.Ensure starter (sandbox "test") (workingIn "/a")
                match! sandboxes.Ensure starter (sandbox "test") (workingIn "/b") with
                | Ok _ -> failwith "expected a refusal"
                | Error e ->
        ...
        testCaseAsync "the spec that was asked for is what the sandbox is built from" <|
            async {
                ...
                let! _ = sandboxes.Ensure starter (sandbox "test") (workingIn "/somewhere")
                Expect.equal
        ...
        testCaseAsync "stopping releases the name, and the next start may differ" <|
            async {
                ...
                let! restarted = sandboxes.Ensure starter (sandbox "test") (forwarding [ "github" ])
                Expect.equal
        ...
        testCaseAsync "stopping default keeps the name reachable" <|
            async {
                let log = newLog ()
                let! sandboxes, _ = registry log []
                let! _ = sandboxes.Ensure starter SandboxRef.defaultRef (forwarding [])
        ...
        testCaseAsync "a bare name finds the one repo sandbox that carries it" <|
            async {
                let log = newLog ()
                let! sandboxes, _ = registry log []
                let! _ = sandboxes.Ensure starter (sandbox "octo/hello:dev") SandboxRequest.defaults
        ...
        testCaseAsync "a bare name two repos both carry is refused, naming both" <|
            async {
                ...
                let! sandboxes, _ = registry log []
                let! _ = sandboxes.Ensure starter (sandbox "octo/hello:dev") SandboxRequest.defaults
                let! _ = sandboxes.Ensure starter (sandbox "octo/world:dev") SandboxRequest.defaults
                match! (sandboxes.EnvironmentFor (sandbox "dev")).Ensure None "a terminal was opened" with
                | EnvironmentAvailable -> failwith "expected a refusal"
                | EnvironmentUnavailable reason ->
        ...
        testCaseAsync "an unknown name is refused naming the sandboxes that exist" <|
            async {
                let log = newLog ()
                let! sandboxes, _ = registry log []
                let! _ = sandboxes.Ensure starter (sandbox "octo/hello:gate") SandboxRequest.defaults
                match! (sandboxes.EnvironmentFor (sandbox "dev")).Ensure None "a terminal was opened" with
                | EnvironmentAvailable -> failwith "expected a refusal"
                | EnvironmentUnavailable reason ->
        ...
        testCaseAsync "a start records where this host could not give what the sandbox asked for" <|
            async {
                ...
                let! _ = sandboxes.Ensure starter (sandbox "test") SandboxRequest.defaults
                let! events = eventsOf log
                match startedEvents events with
                | [ started ] ->
                    Expect.equal
    ...
    testList "named credential forwarding" [
        ...
        testCaseAsync "the provision reaches the sandbox env, and the event carries names only" <|
            async {
                ...
                let! started = sandboxes.Ensure starter (sandbox "test") (forwarding [ "github" ])
                Expect.equal
        ...
        testCaseAsync "a sandbox that forwards github starts with nobody's credential named" <|
            async {
                ...
                let! started = sandboxes.Ensure (Authority.configuredBy repo CredentialFor.Deployment) (sandbox "octo/hello:dev") (forwarding [ "github" ])
                match started with
        ...
        // The file is the author, but somebody asked for the fold that started it. The
        // start records who, so a note can say it was for them.
        testCaseAsync "a repo's sandbox started for a person records that person" <|
            async {
                let log = newLog ()
                let! sandboxes, _ = registry log []
                let repo = RepoRef.create "octo/hello" |> expect
                let ada = Principal.User (UserId.create "ada" |> expect)
                let! _ = sandboxes.Ensure (Authority.configuredBy repo (CredentialFor.Person ada)) (sandbox "octo/hello:dev") (forwarding [])
                let! events = eventsOf log
                match startedEvents events with
                | [ e ] -> Expect.equal e.OnBehalfOf (Some ada) "for Ada"
                | other -> failwithf "expected one start, got %A" other
            }

        // At boot nobody asked: the deployment folds on its own, so nobody is named.
        testCaseAsync "a repo's sandbox started at boot records nobody behind it" <|
            async {
                let log = newLog ()
                let! sandboxes, _ = registry log []
                let repo = RepoRef.create "octo/hello" |> expect
                let! _ = sandboxes.Ensure (Authority.configuredBy repo CredentialFor.Deployment) (sandbox "octo/hello:dev") (forwarding [])
                let! events = eventsOf log
                match startedEvents events with
                | [ e ] -> Expect.isNone e.OnBehalfOf "for nobody"
                | other -> failwithf "expected one start, got %A" other
            }
        ...
        testCaseAsync "a credential this session does not know is refused, naming the ones it does" <|
            async {
                let log = newLog ()
                let! sandboxes, _ = registry log [ githubCredential "tok" ]
                match! sandboxes.Ensure starter (sandbox "test") (forwarding [ "gitlab" ]) with
                | Ok _ -> failwith "expected a refusal"
                | Error e ->
    ...
    testList "the work_sandboxes query" [
        ...
        testCaseAsync "a repo's sandbox says which repo declared it, and the session's says nothing" <|
            async {
                ...
                let! _ = sandboxes.Ensure starter declared SandboxRequest.defaults
                match! registration.Read () with
                | Error e -> failwithf "the query failed: %s" e
                | Ok (RowsOf rows) ->
