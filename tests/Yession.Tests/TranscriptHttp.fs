...
let private endpointTests =
    testList "HTTP endpoint" [
        ...
        testCaseAsync "a range answers the lines it names, and the client keeps them" <|
            async {
                let! at, stop = serving 10
                let! answer = TestHttp.get (at (TerminalTranscriptRange (TerminalId.value terminal, 0, 10)) token)
                Expect.equal answer.Status 200 "the range serves"
                Expect.equal (lines answer.Body).Length 11 "the header and ten records"
                Expect.equal (TestHttp.requiredHeader "cache-control" answer) "no-store" "the client keeps this, not the HTTP cache"
                do! stop ()
            }
            ...
            async {
                ...
                let url =
                    sprintf
                        ...
                        token
                let! before = TestHttp.get url
                for i in 5 .. 20 do
                    transcript.Append { At = 0.0; Kind = TranscriptOutput; Data = sprintf "line %d" i } |> ignore
                let! after = TestHttp.get url
        ...
        testCaseAsync "a range the transcript has not reached does not exist yet" <|
            async {
                ...
                let! at, stop = serving 10
                let! unreached = TestHttp.get (at (TerminalTranscriptRange (TerminalId.value terminal, 100, 199)) token)
        ...
        testCaseAsync "both legs are gated on minted tokens" <|
            async {
                let! at, stop = serving 10
                let! cursor = TestHttp.get (at (TerminalTranscriptAfter (TerminalId.value terminal, None)) "stolen")
                Expect.equal cursor.Status 401 "the cursor is gated"
                let! range = TestHttp.get (at (TerminalTranscriptRange (TerminalId.value terminal, 0, 10)) "stolen")
