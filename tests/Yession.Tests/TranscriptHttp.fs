    ...
    testList "HTTP endpoint" [
        testCaseAsync "a cursor with no position redirects to the range that starts the recording" <|
            async {
                ...
                let! start = httpGetRaw (at (TerminalTranscriptAfter (TerminalId.value terminal, None)) token) |> Interop.awaitPromise
        ...
        testCaseAsync "the range a cursor resolves to begins one line past it" <|
            async {
                ...
                let! at, stop = serving 10
                let! start = httpGetRaw (at (TerminalTranscriptAfter (TerminalId.value terminal, Some 4)) token) |> Interop.awaitPromise
                Expect.stringContains
                    start.location
        ...
        testCaseAsync "a range answers the lines it names, and the client keeps them" <|
            async {
                let! at, stop = serving 10
                let! answer = httpGet (at (TerminalTranscriptRange (TerminalId.value terminal, 0, 10)) token) |> Interop.awaitPromise
            ...
            async {
                ...
                let url =
                    sprintf
                        ...
                        token
                let! before = httpGet url |> Interop.awaitPromise
                for i in 5 .. 20 do
                    transcript.Append { At = 0.0; Kind = TranscriptOutput; Data = sprintf "line %d" i } |> ignore
                let! after = httpGet url |> Interop.awaitPromise
        ...
        testCaseAsync "a range the transcript has not reached does not exist yet" <|
            async {
                ...
                let! at, stop = serving 10
                let! unreached = httpGet (at (TerminalTranscriptRange (TerminalId.value terminal, 100, 199)) token) |> Interop.awaitPromise
        ...
        testCaseAsync "a caller at the tail is told it is current, not given an empty range" <|
            async {
                ...
                let! at, stop = serving 10
                let! current = httpGetRaw (at (TerminalTranscriptAfter (TerminalId.value terminal, Some 10)) token) |> Interop.awaitPromise
        ...
        testCaseAsync "a terminal with no recording answers its cursor rather than refusing it" <|
            async {
                ...
                let! at, stop = serving 10
                let! unknown = httpGetRaw (at (TerminalTranscriptAfter ("term-never-opened", None)) token) |> Interop.awaitPromise
        ...
        testCaseAsync "both legs are gated on minted tokens" <|
            async {
                let! at, stop = serving 10
                let! cursor = httpGet (at (TerminalTranscriptAfter (TerminalId.value terminal, None)) "stolen") |> Interop.awaitPromise
                Expect.equal cursor.status 401 "the cursor is gated"
                let! range = httpGet (at (TerminalTranscriptRange (TerminalId.value terminal, 0, 10)) "stolen") |> Interop.awaitPromise
