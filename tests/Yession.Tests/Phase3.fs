    ...
    testList "Process doc persistence (Step 19)" [
        testCaseAsync "a restart replays the persisted doc: pending entries drain exactly once at boot; drafts survive and are not re-announced" <|
            async {
                ...
                let! h1 = Host.startFull (fun () -> Some runner1) (fun () -> None) None None (Some (openLog ())) (Some (DocStore.openStore docPath)) None None None (fun _ _ -> ()) None McpClient.McpConnections.none None sessionId None "" None false None 0
                ...
                let! h2 = Host.startFull (fun () -> Some runner2) (fun () -> None) None None (Some (openLog ())) (Some (DocStore.openStore docPath)) None None None (fun _ _ -> ()) None McpClient.McpConnections.none None sessionId None "" None false None 0
                let! secondLife = sentMessages h2.Log
                Expect.equal
        ...
        testCaseAsync "an empty draft slot in the persisted doc is dropped at boot; a typed draft is not" <|
            async {
                ...
                let! h1 = Host.startFull (fun () -> None) (fun () -> None) None None (Some (openLog ())) (Some (DocStore.openStore docPath)) None None None (fun _ _ -> ()) None McpClient.McpConnections.none None sessionId None "" None false None 0
                ...
                let! h2 = Host.startFull (fun () -> None) (fun () -> None) None None (Some (openLog ())) (Some (DocStore.openStore docPath)) None None None (fun _ _ -> ()) None McpClient.McpConnections.none None sessionId None "" None false None 0
                let synced = SyncedStateSync.ofDoc h2.Doc |> Result.mapError (sprintf "%A") |> expect
