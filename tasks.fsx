...
let private runCheckOnce (requested: string list) (runtime: Runtime option) =
    ...
    if capSet.Contains "Browser" && (runtime = None || runtime = Some Runtime.Clr) then
        ...
        // does not exist, so every browser test errors with "No such file or directory" on
        // the driver's node before Chromium is ever reached (seen in a real session: 46
        // browser tests, all errored). PLAYWRIGHT_BROWSERS_PATH does not help — that is the
        // browser, this is the driver's own node. Playwright's own override for exactly this
        // is PLAYWRIGHT_NODEJS_PATH: the node the driver spawns instead of the shipped one
        // (it wins over the `.playwright/node/<rid>/node` path the driver otherwise guesses
        // from the assembly's location). The dev shell's `node` runs wherever this task does
        // (the sandbox, CI, a laptop), so point the driver at it; a box with no `node` on
        // PATH sets nothing and keeps the shipped binary, behaving as before.
...
// --- brand -----------------------------------------------------------------------------------
// The mark, as the product carries it: `src/Yession.App/Brand.fs` is GENERATED from
// `assets/logo` (the reference set, itself generated by `assets/logo/gen.py`), so the bytes a
// session serves for its icon and its tab are the reference set's by construction and never
// a copy somebody pasted. A cheap-tier test (`Phase4`) reads both and says when they have
// drifted, which is what makes running this after regenerating the assets a thing you cannot
// forget rather than a thing you have to remember.

let brandFile = Path.Combine (repoRoot, "src", "Yession.App", "Brand.fs")

let brandSource () =
    let asset name = Path.Combine (repoRoot, "assets", "logo", name)
    let svg name =
        let text = (File.ReadAllText (asset name)).Trim ()
        if text.Contains "\"\"\"" then failwithf "%s contains a triple quote and cannot be a triple-quoted literal" name
        if text.Contains "{" || text.Contains "}" then failwithf "%s contains a brace, which an interpolated template would read as a hole" name
        text
    // ONE literal, continued with backslashes so the file diffs line by line rather than as
    // a single 160KB line — and not a chain of `+`, which the compiler would nest 1,500 deep.
    let iconPng = Convert.ToBase64String (File.ReadAllBytes (asset "icon-512.png"))
    let wrapped =
        [ for i in 0 .. 100 .. iconPng.Length - 1 -> "          " + iconPng.Substring (i, min 100 (iconPng.Length - i)) ]
        |> String.concat "\\\n"
    String.concat "\n" [
...
let brand () =
    File.WriteAllText (brandFile, brandSource ())
    printfn "wrote %s" brandFile
